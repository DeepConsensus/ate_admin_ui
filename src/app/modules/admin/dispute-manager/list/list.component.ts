...
export class ListComponent implements OnInit, OnDestroy {
  ...
  public dataSource = new MatTableDataSource<DisputeManagerModel>();
  length = 0;
  pageSize = 10;
  pageIndex = 0;
    ...
    this.service
      ...
      .subscribe((data: any) => {
        if (data && data.docs) {
          ...
          this.dataSource.data = data.docs as DisputeManagerModel[];
  ...
  editRow(): void {
    if (this.selectedRowItem) {
      this._router.navigate([
        "/inventory/dispute-manager/update/" + this.selectedRowItem._id,
      ]);
    }
  }
    ...
    if (this.selectedRowItem) {
      this.service.delete(this.selectedRowItem._id).subscribe((data: any) => {
        if (data.errors) {
          ...
          this.openSnackBar("Dispute Manager has been deleted successfully.");
          this.getList();
        }
      });
    }
