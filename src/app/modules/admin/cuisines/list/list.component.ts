...
export class ListComponent implements OnInit, OnDestroy {
  ...
  public dataSource = new MatTableDataSource<CuisinesModel>();
  length = 0;
  pageSize = 10;
  pageIndex = 0;
    ...
    this.service
      ...
      .subscribe((data: any) => {
        if (data && data.docs) {
          ...
          this.dataSource.data = data.docs as CuisinesModel[];
  ...
  editRow(): void {
    if (this.selectedRowItem) {
      this._router.navigate([
        "/inventory/cuisines/update/" + this.selectedRowItem._id,
      ]);
    }
  }
    ...
    if (this.selectedRowItem) {
      this.service.delete(this.selectedRowItem._id).subscribe((data: any) => {
        if (data.errors) {
          ...
          this.openSnackBar("Cuisine has been deleted successfully.");
          this.getList();
        }
      });
    }
