...
import { DisputeManager } from 'app/modules/admin/dispute-manager/dispute-manager.types';
...
export class ListComponent implements OnInit, OnDestroy {
  ...
  public dataSource = new MatTableDataSource<DisputeManager>();
  length = 0;
  pageSize = 10;
  pageIndex = 0;
    ...
    this.service
      ...
      .subscribe((data: any) => {
        if (data && data.docs) {
          ...
          this.dataSource.data = data.docs as DisputeManager[];
  ...
  editRow(): void {
    if (this.selectedRowItem) {
      ...
      this._router.navigate([
        "/delivery-people/update/" + this.selectedRowItem._id,
      ]);
    }
  }
    ...
    if (this.selectedRowItem) {
      this.service.delete(this.selectedRowItem._id).subscribe((data: any) => {
        if (data.errors) {
          ...
          this.openSnackBar("Delivery Person has been deleted successfully.");
          this.getList();
        }
      });
    }
