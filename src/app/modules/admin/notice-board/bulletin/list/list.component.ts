...
import { Notice } from "app/modules/admin/notice-board/bulletin/notice.types";
...
export class ListComponent implements OnInit, OnDestroy {
  public columns: string[] = ["slno", "name", "notice", "createdAt", "action"];
  //public dataSource: any[] = [];
  public dataSource = new MatTableDataSource<Notice>();
  length = 0;
  pageSize = 10;
  pageIndex = 0;
    ...
    this.service
      ...
      .subscribe((data: any) => {
        if (data && data.docs) {
          ...
          this.dataSource.data = data.docs as Notice[];
  ...
  editRow(): void {
    if (this.selectedRowItem) {
      ...
      this._router.navigate([
        "/inventory/noticeboard/update/" + this.selectedRowItem._id,
      ]);
    }
  }
    ...
    if (this.selectedRowItem) {
      this.service.delete(this.selectedRowItem._id).subscribe((data: any) => {
        if (data.errors) {
          ...
          this.openSnackBar("Notice Board has been deleted successfully.");
          this.getList();
        }
      });
    }
