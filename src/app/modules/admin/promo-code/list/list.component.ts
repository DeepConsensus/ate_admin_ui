...
export class ListComponent implements OnInit, OnDestroy {
  public columns: string[] = ["slno", "promocode", "type", "discount", "expired_date", "promocodetype", "action"];
  //public dataSource: any[] = [];
  public dataSource = new MatTableDataSource<PromoCode>();
  length = 0;
  pageSize = 10;
  pageIndex = 0;
  ...
  ngOnInit(): void {
    ...
    this.dataSource.filterPredicate = (
      ...
      filterValue: string
    ) => data.promocode.trim().toLowerCase().indexOf(filterValue) !== -1;
  }
    ...
    this.service
      ...
      .subscribe((data: any) => {
        if (data && data.docs) {
          ...
          this.dataSource.data = data.docs as PromoCode[];
  ...
  editRow(): void {
    if (this.selectedRowItem) {
      ...
      this._router.navigate([
        "/inventory/promocode/update/" + this.selectedRowItem._id,
      ]);
    }
  }
    ...
    if (this.selectedRowItem) {
      this.service.delete(this.selectedRowItem._id).subscribe((data: any) => {
        if (data.errors) {
          ...
          this.openSnackBar("Promotional Code has been deleted successfully.");
          this.getList();
        }
      });
    }
